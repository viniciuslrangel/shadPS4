...
void Translator::S_CMP(ConditionOp cond, bool is_signed, const GcnInst& inst) {
    const auto lhs = GetSrc<IR::U32>(inst.src[0]);
    const auto rhs = GetSrc<IR::U32>(inst.src[1]);
    const IR::U1 result = [&] {
        switch (cond) {
        case ConditionOp::EQ:
...
void Translator::S_BFM_B32(const GcnInst& inst) {
    const IR::U32 src0{ir.BitwiseAnd(GetSrc<IR::U32>(inst.src[0]), ir.Imm32(0x1F))};
...
void Translator::S_BREV_B32(const GcnInst& inst) {
    SetDst(inst.dst[0], ir.BitReverse(GetSrc<IR::U32>(inst.src[0])));
}
